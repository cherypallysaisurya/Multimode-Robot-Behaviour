...
class RobotInterface:
    ...
    def _setup_simulator(self):
        ...
        try:
            # Import from local simulator package
            from .simulator import create_robot_program
    ...
    def _setup_real_robot(self):
        ...
        try:
            from .robot_hardware import Dog, Mode
            
            print("🔄 Connecting to robot...")
            
            # Create robot connection (includes automatic fallback to mock)
            self._dog = Dog(ROBOT_SETTINGS["host"])
            
            if not self._dog.is_mock:
                # Only do initialization sequence for real robot
                ...
                print(f"🧪 Using mock robot mode (host: {ROBOT_SETTINGS['host']})")
    ...
    def _setup_mock_robot(self):
        """Setup mock robot for development/testing"""
        from .robot_hardware import MockDog
...
def run_with_visualization(program: RobotProgram, moves_function, move_delay: float = None):
    ...
    if program.mode == "simulator":
        ...
        try:
            from .simulator import run_with_visualization as sim_viz
